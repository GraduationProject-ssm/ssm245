...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1gn8XeNE2J?p=43)


# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本个性化美食推荐系统也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让医生管理信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，个性化美食推荐系统的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，个性化美食推荐系统是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此个性化美食推荐系统为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
个性化美食推荐系统的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
图4.8 系统E-R图
### 4.3.2 数据库表结构设计
数据库系统一旦选定之后，需要根据程序要求在数据库中建立数据库文件，并在已经完成创建的数据库文件里面，为程序运行中产生的数据建立对应的数据表格，数据表结构设计就是对创建的数据表格进行字段设计，字段长度设计，字段类型设计等，当数据表格合理设计完成之后，才能正常存储相关程序运行产生
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|8|meishi\_name|String|美食名称|是|
|9|meishi\_money|BigDecimal|美食价格|是|
|10|kouwei\_types|Integer|口味|是|
|11|dianzhi\_types|Integer|店址|是|
|12|meishi\_photo|String|图片|是|
|13|meishi\_jieyue|Integer|点击率|是|
|14|meishi\_content|String|详情信息|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|16|meishi\_id|Integer|美食id|是|
|17|yonghu\_id|Integer|用户id|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|20|meishi\_id|Integer|美食id|是|
|21|yonghu\_id|Integer|用户id|是|
|22|meishi\_liuyan\_content|String|留言内容|是|
|23|reply\_content|String|回复内容|是|
|24|insert\_time|Date|讨论时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|26|meishi\_id|Integer|美食id|是|
|27|yonghu\_id|Integer|用户id|是|
|28|order\_number|Integer|预定数量|是|
|29|insert\_time|Date|订单创建时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|31|news\_name|String|博主推荐名称|是|
|32|news\_photo|String|博主推荐图片|是|
|33|insert\_time|Date|博主推荐时间|是|
|34|news\_content|String|博主推荐详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|36|yonghu\_name|String|用户姓名|是|
|37|sex\_types|Integer|性别|是|
|38|yonghu\_id\_number|String|身份证号|是|
|39|yonghu\_phone|String|手机号|是|
|40|yonghu\_photo|String|照片|是|
|41|kouwei\_types|Integer|口味|是|
|42|dianzhi\_types|Integer|住址|是|
|43|new\_money|BigDecimal|余额|是|
...
图5.1 用户信息管理页面
### 5.2 美食信息管理
如图5.2显示的就是美食信息管理页面，此页面提供给管理员的功能有：查看已发布的美食信息数据，修改美食信息，美食信息作废，即可删除。

![](/md/blog.011.png)

图5.2 美食信息管理页面
### 5.3美食留言管理
如图5.3显示的就是美食留言管理页面，此页面提供给管理员的功能有：根据美食留言进行条件查询，还可以对美食留言进行新增、修改、查询操作等等。
...
图5.3 美食留言管理页面
### 5.1博主推荐管理
如图5.4显示的就是博主推荐管理页面，此页面提供给管理员的功能有：根据博主推荐进行新增、修改、查询操作等等。
